...
const HomeNavbarv2 = (props) => {
    ...
    const searchBgColor = colorScheme === 'dark' ? '#262729' : '#f4f5f7';
    const searchBdColor = colorScheme === 'dark' ? '#323335' : '#dee2e6';
    const kdbBdColor = colorScheme === 'dark' ? '#202020' : '#b2b3b5';

    return (
        <nav className="navbar w-100 fixed h-16	top-0" style={{zIndex: "11", backgroundColor: themeColors.bg[2],
                    ...
                    <Grid.Col span={10} >
                        <Flex align='center' justify='end'>
                            <Box pe={{base: 3, xs: 12}}>
                                ...
                                <button className={`flex py-1 ps-3 pe-2.5 rounded gap-32 border-solid border-[${searchBdColor}]`} 
                                style={{border: `1px solid ${searchBdColor}`, backgroundColor: `${searchBgColor}`}}>
                                    <Flex ff='Inter' fz={13.5} align='center' className='text-muted-foreground'>Search...</Flex>
                                    <Flex ms={10} p='2px 9px'  h={28} bg={inputRightSection} className='rounded-md' bd={`.5px solid ${searchBdColor}`} >
                                        <Flex align='center' pb={1} >
                                            {Icons('IconCommand',15,15,themeColors.text[3])}
                                        </Flex>
                                        <Text m='auto' c={themeColors.text[3]} ff='Inter' fw={600} fz={13}>K</Text>
